...
class ProductRepository:
    ...
    def get_product_by_permalink(self, permalink: str):
        query = text("""
        SELECT
            p.toy_id,
            p.name,
        ...
        GROUP BY
            ...
            ps.quantity
        """)
        row = self.db.exec(query, {"permalink": permalink}).first()
        return row if row else None

    def get_product_by_id(self, toy_id: int):
        query = text("""
        SELECT
            p.toy_id,
            p.name,
        ...
        GROUP BY
            ...
            ps.quantity
        """)
        row = self.db.exec(query, {"toy_id": toy_id}).first()
